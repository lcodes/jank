...
ASSET_FORMAT(Texture, bmp);
ASSET_FORMAT(Texture, gif);
ASSET_FORMAT(Texture, jpg);
ASSET_FORMAT(Texture, jpeg);
ASSET_FORMAT(Texture, png);
ASSET_FORMAT(Texture, psd);

ASSET_FORMAT(Texture, hdr);

static Type type{ "Texture" };

Type const& TextureImporter::getType() const { return type; }

bool TextureImporter::process() {
  return true;
}
